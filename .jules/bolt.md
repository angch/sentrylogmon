...
## 2026-05-17 - Replacing time.Parse with manual parsing
**Learning:** Replaced time.Parse with manual byte array parsing using atoi4 and atoi2 for ParseNginxError, avoiding string allocations prior to successful bounds-checking.
**Action:** When working on log parsers with strictly defined formats, manual parsing and character index checks are significantly faster and reduce GC pressure compared to time.Parse and string casts.

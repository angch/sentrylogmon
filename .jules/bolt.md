...
## 2026-01-27 - time.Parse Overhead
**Learning:** Using `time.Parse` in hot paths like log parsing is significantly slower than manual character extraction and using `time.Date`, primarily due to parsing format overhead and allocations.
**Action:** For simple, fixed-format timestamps in log detectors, prefer using manual parsing helpers (like `atoi4`/`atoi2`) and `time.Date` directly. Ensure you strictly bounds-check the extracted values since `time.Date` silently normalizes out-of-bounds dates.

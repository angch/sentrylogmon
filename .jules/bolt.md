...
## 2026-02-12 - Nginx Error Timestamp Optimization
**Learning:** `time.Parse` incurs overhead due to layout string parsing and reflection, which becomes a bottleneck in hot paths processing fixed-format timestamps like Nginx error logs.
**Action:** Replace `time.Parse` with manual byte extraction using local helpers (`atoi2`, `atoi4`) and `time.Date` for fixed-format logs. Validate all components manually to prevent silent normalizations.

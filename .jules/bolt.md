...
## 2025-02-09 - Optimize Nginx error timestamp parsing
**Learning:** Standard library functions like `time.Parse` can introduce significant reflection and allocation overhead.
**Action:** Replace `time.Parse` with manual `time.Date` and indexing byte extraction for fixed-format timestamps in hot paths.

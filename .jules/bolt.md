...
## 2026-04-04 - Fast-path string extraction in JSON Detector
**Learning:** Using `fmt.Sprintf("%v", val)` on `any` values causes allocations and is slow (~175ns). Type assertions (`if s, ok := val.(string); ok`) have zero allocations and are orders of magnitude faster (~1.5ns).
**Action:** Always attempt a type assertion to `string` first before falling back to `fmt.Sprintf` when extracting string representations from `interface{}` values in hot paths.

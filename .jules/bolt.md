...
## 2024-06-14 - Optimize time.Parse for Nginx Error Logs
**Learning:** Using time.Parse with standard layout strings introduces reflection and string allocation overhead compared to manual byte parsing and `time.Date()`.
**Action:** Continue to replace `time.Parse` with `time.Date` alongside custom `atoi2` and `atoi4` helpers for fixed-format timestamp parsing in hot paths, ensuring strict bound checks.

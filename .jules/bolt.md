...
## 2026-05-21 - time.Parse Overhead in Hot Paths
**Learning:** `time.Parse` incurs high CPU overhead on hot paths because it uses generalized layout parsing.
**Action:** When parsing strictly formatted logs (like Nginx), use manual byte scanning and helpers like `atoi2`/`atoi4` to extract components, then construct `time.Time` directly with `time.Date(..., time.UTC)`.

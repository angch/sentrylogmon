...
func ParseISO8601(line []byte) (float64, string, bool) {
	...
	// Optimization: Extract components directly using atoi2/atoi4 and bounds-check them.
	// This avoids the reflection and string allocation overhead of time.Parse in this hot path.
	y := atoi4(line[0:4])
	m := atoi2(line[5:7])
	d := atoi2(line[8:10])
	h := atoi2(line[11:13])
...
func ParseNginxError(line []byte) (float64, string, bool) {
	...
	// Optimization: Allocate string only after successful validation.
	return float64(t.Unix()), string(line[:19]), true
}

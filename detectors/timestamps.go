...
func ParseNginxError(line []byte) (float64, string, bool) {
	...
	minute := atoi2(line[14:16])
	sec := atoi2(line[17:19])

	if year < 0 || month < 1 || month > 12 || day < 1 || day > 31 || hour < 0 || hour > 23 || minute < 0 || minute > 59 || sec < 0 || sec > 60 {
		return 0, "", false
	}

	t := time.Date(year, time.Month(month), day, hour, minute, sec, 0, time.UTC)
	return float64(t.Unix()) + float64(t.Nanosecond())/1e9, string(line[:19]), true
}

...
func ParseNginxError(line []byte) (float64, string, bool) {
	...
	// Optimization: Manually construct time.Date instead of allocating via time.Parse to eliminate reflection overhead
	t := time.Date(y, time.Month(m), d, h, min, s, 0, time.UTC)
	tsStr := string(line[:19])
	// Nanosecond is 0, so we can return directly

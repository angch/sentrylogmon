...
func NewJsonDetector(pattern string) (*JsonDetector, error) {
	...
	// Pre-calculate the JSON-encoded field name for fast-path rejection
	// json.Marshal adds quotes which handles spaces well: "field"
...
func (d *JsonDetector) Detect(line []byte) bool {
	// Fast-path: reject lines that don't contain the target field at all
	...
	// Fast-path: if the value is already a string, avoid the heavy fmt.Sprintf allocation
	var valStr string

...
func printInstanceTable(out io.Writer, instances []ipc.StatusResponse) {
	...
	fmt.Fprintln(w, "STATUS \t     PID\tSTARTED\t      UPTIME\t       MEM\tVERSION\tMONITORS")
	for _, inst := range instances {
		...
		status := "Running"
		fmt.Fprintf(w, "%s\t%8d\t%s\t%12s\t%10s\t%s\t%s\n", status, inst.PID, inst.StartTime.Format("2006-01-02 15:04:05"), uptimeStr, memStr, version, details)
	}
	w.Flush()
}

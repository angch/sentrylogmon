...
func printInstanceTable(w io.Writer, instances []ipc.StatusResponse) {
	...
	fmt.Fprintln(tw, "STATUS    \t     PID\tSTARTED\t      UPTIME\t       MEM\tVERSION\tMONITORS")
	for _, inst := range instances {
		...
		fmt.Fprintf(tw, "%s\t%8d\t%s\t%12s\t%10s\t%s\t%s\n", "🔵 Running", inst.PID, inst.StartTime.Format("2006-01-02 15:04:05"), uptimeStr, memStr, version, details)
	}
	tw.Flush()
}

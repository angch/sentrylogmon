...
pub fn get_socket_dir() -> std::path::PathBuf {
    #[cfg(unix)]
    return std::path::PathBuf::from(format!("/tmp/sentrylogmon-{}", unsafe { libc::getuid() }));
    #[cfg(not(unix))]
    return std::path::PathBuf::from("/tmp/sentrylogmon");
}
...
async fn main() -> Result<()> {
    ...
    if cfg.status {
        // SECURITY: Prevent Local DoS by namespacing the IPC socket directory per user

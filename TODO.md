...
- [x] **Prometheus Metrics for Monitor Lag** (2026-03-26)
  - Implemented `MonitorLag` prometheus metric in `metrics.go`.
  - Updated `Monitor` struct to cache the metric per source.
  - Calculated and recorded lag in `processMatch()` ignoring negative values.

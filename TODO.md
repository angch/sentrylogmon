...
- [x] **Prometheus Metrics for Monitor Lag**
  - Track time difference between log timestamp and processing time in `metrics.MonitorLagSeconds`.
  - Ignored negative lag values and handled relative timestamps correctly with `host.BootTime()`.
  - Added new test file `monitor_lag_test.go` to verify logic.

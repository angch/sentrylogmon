...
- [x] **Prometheus Metrics for Monitor Lag**
  - Track time difference between log timestamp and processing time (`sentrylogmon_monitor_lag_seconds` as a `GaugeVec`).
  - Added unit tests `TestMonitorLag` and `TestMonitorLag_NegativeLagIgnored` using unique labels.
  - Implemented logic in `monitor.processMatch` to ignore negative lag and evaluate `UnixNano` accurately.

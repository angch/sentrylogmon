...
- [x] **Prometheus Metrics for Monitor Lag** (2026-02-21)
  - Implemented `MonitorLagSeconds` histogram metric.
  - Added periodic lag calculation in `Monitor.Start` loop (every 1s).
  - Used `tryExtractTimestamp` to leverage existing extractors.
  - Added comprehensive test `TestMonitorLag`.

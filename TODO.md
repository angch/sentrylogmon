...
- [x] **Prometheus Metrics for Monitor Lag** (2026-02-08)
  - Implemented `sentrylogmon_monitor_lag_seconds` gauge in `metrics/metrics.go`.
  - Added calculation in `monitor.processMatch` properly handling both absolute and relative dmesg timestamps using `host.BootTime()`.
  - Safely ignores negative lag values to avoid corruption from clock skew.
  - Added tests checking positive lag values and skipping of negative lags.

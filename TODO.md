...
- [x] **Prometheus Metrics for Monitor Lag** (2026-04-08)
  - Implemented `MonitorLagSeconds` in `metrics.go` to track lag between log event occurrence and processing time.
  - Added timestamp subtraction logic and `metricMonitorLag.Set(lag)` to `monitor.go`.
  - Added unit test `TestMonitorLagMetric` to ensure lag computations and metric reporting function correctly.

...
- [x] **Prometheus Metrics for Monitor Lag** (2026-03-21)
  - Implemented `MonitorLagSeconds` HistogramVec to track lag between log extraction and processing.
  - Mitigated clock skew related anomalies by adding `bootTime` tracking to compare with `host.BootTime()` and ignoring lag differences that result in negative values.
  - Updated `monitor.go` implementation logic.
  - Added new assertions and verified with tests in `monitor/observability_test.go`.

...
- [x] **Prometheus Metrics for Monitor Lag** (2026-02-09)
  - Added `MonitorLag` histogram to `metrics.go`.
  - Calculated lag `float64(time.Now().UnixNano())/1e9 - timestamp` in `monitor.go`'s `processMatch`.
  - Included a test to verify the metric recording.

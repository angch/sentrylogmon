...
- [x] **Prometheus Metrics for Monitor Lag** (2026-02-09)
  - Added `MonitorLagSeconds` histogram metric in `metrics/metrics.go`.
  - Implemented lag tracking in `processMatch` handling both Unix timestamps and dmesg uptime correctly.
  - Added unit tests in `monitor/lag_test.go` for metric collection.

...
- [x] **Prometheus Metrics for Monitor Lag** (2026-03-17)
  - Added `MonitorLagSeconds` GaugeVec metric to track parsing lag.
  - Implemented lag calculation in `processMatch()` that prevents clock skew data corruption by ignoring negative lag.
  - Added test coverage in `monitor_lag_test.go` utilizing isolated registry state mapping.

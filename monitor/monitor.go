...
func (m *Monitor) processMatch(line []byte) {
	...
	if timestamp > 0 {
		// Calculate lag
		// timestamp is in seconds (float64). Convert to time.Time
		tsSec := int64(timestamp)
		tsNsec := int64((timestamp - float64(tsSec)) * 1e9)
		logTime := time.Unix(tsSec, tsNsec)

		lag := m.lastActivityTime.Sub(logTime).Seconds()
		if lag > 0 {

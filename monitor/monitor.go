...
func (m *Monitor) processMatch(line []byte) {
	...
	if timestamp > 0 {
		// Time diff based on lastActivityTime (sub-second precision)
		lag := float64(m.lastActivityTime.UnixNano())/1e9 - timestamp

		// If the timestamp is relative (like dmesg) or from the future (clock skew), we might get negative lag.
		// Also dmesg timestamps < 10 years are considered relative to boot time in our detectors.
		// Ignore negative lag to avoid data corruption.
		if lag >= 0 {

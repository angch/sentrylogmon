...
fn sanitizeCommand(allocator: std.mem.Allocator, args: []const []const u8) ![]u8 {
    ...
    for (args, 0..) |arg, i| {
        ...
        if (std.mem.indexOf(u8, arg, "=")) |eq_idx| {
            ...
            const lower_key_check = try std.ascii.allocLowerString(allocator, key);
            defer allocator.free(lower_key_check);

            if (sensitive or sensitive_flags.has(lower_key_check)) {

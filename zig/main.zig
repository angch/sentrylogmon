...
pub fn main() !void {
    ...
    // IPC Commands
    // SECURITY: Prevent Local DoS by using user-specific runtime directory instead of shared /tmp
    const socket_dir = if (@import("builtin").os.tag != .windows) blk: {
        if (std.posix.getenv("XDG_RUNTIME_DIR")) |xdg| {
            break :blk try std.fmt.allocPrint(allocator, "{s}/sentrylogmon", .{xdg});
        } else {
            const uid = std.posix.getuid();
            break :blk try std.fmt.allocPrint(allocator, "/tmp/sentrylogmon-{d}", .{uid});
        }
    } else blk: {
        break :blk try std.fmt.allocPrint(allocator, "/tmp/sentrylogmon", .{});
    };
